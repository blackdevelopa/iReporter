...
const redFlag = {
   ...
   */
   ...
   async getRedflagsById(req, res) {
     const findQueryById = 'SELECT * FROM incidents WHERE id=$1 AND createdBy = $2';
     try {
       const { rows } = await db.query(findQueryById, [req.params.id, req.user.id]);
       if(!rows[0]) {
         return res.status(400).json({
           status: 400,
    ...
    */
    ...
    async patchRedflagLocation(req, res) {
      const findOneQuery = 'SELECT * FROM incidents WHERE id=$1 AND createdBy=$2';
