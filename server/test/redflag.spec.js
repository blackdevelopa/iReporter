...
it('should return all available red-flags records', (done) => {
	chai.request(app)
...
it('should return a redflag record with a specific id', (done) => {
	chai.request(app)
...
it('should return a intervention record with a specific id', (done) => {
	...
	expect(res.body.status).to.be.equals(400);
	expect(res.body.message).to.be.a('string');
			expect(res.body.message).to.equals('There is no redflag here');
			done();
		});
});
});
...
it('should edit the location value of a record if it exists', (done) => {
	chai.request(app)
		.patch('/api/v1/red-flags/32/location')
		.set({ 'x-access-token': token })
		.send({
			location: 'calabar',
		})
		.end((err, res) => {
			...
			expect(res.body.data[0].message).to.equals('Updated red-flags location');
			done();
		});
});
});
...
it('should edit the location value of a record if it exists', (done) => {
	chai.request(app)
		...
		.end((err, res) => {
			...
			expect(res.body.status).to.be.equals(400);
			expect(res.body.message).to.be.a('string');
			expect(res.body.message).to.equals('This redflag does not exist');
			done();
		});
});
});
...
it('should edit the comment value of a record if it exists', (done) => {
	chai.request(app)
		.patch('/api/v1/red-flags/32/comment')
		.set({ 'x-access-token': token })
		.send({
			comment: 'calabar',
		})
		.end((err, res) => {
			...
			expect(res.body.data[0]).to.haveOwnProperty('message');
			expect(res.body.data[0]).to.haveOwnProperty('id' && 'message');
			expect(res.body.data[0].message).to.equals('Updated red-flags comment');
			done();
		});
});
});
...
it('should successfully change the status of a record is the user is an admin', (done) => {
	chai.request(app)
		.patch('/api/v1/red-flags/32/status')
		...
		.end((err, res) => {
			...
			expect(res.body.status).to.be.equals(200);
			expect(res.body).to.haveOwnProperty('data');
			expect(res.body.data[0].message).to.equals('Updated redflag record status');
			done();
		});
});
});
...
it('should delete a record by id if it exists', (done) => {
	chai.request(app)
		...
		.end((err, res) => {
			...
			expect(body.message).to.be.equals('This red-flag does not exist');
			done();
		});
});
});